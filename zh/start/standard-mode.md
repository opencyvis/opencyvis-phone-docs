...
<Callout type="tip">
配对码是输入到 OpenCyvis 的**通知**里，而不是切回 App 输入——这样不需要离开设置界面，配对就不会因为超时而失败。如果没看到通知横幅，下拉通知栏找到 OpenCyvis 的「点此输入配对码」通知即可。
</Callout>

<Callout type="info">
**分屏引导** — 对于支持自动分屏的机型（Android 12L+），OpenCyvis 会自动以分屏方式打开配对界面与无线调试页面并排显示，你可以在看到配对码的同时直接输入，无需切换应用。

**国产 ROM 通知问题** — 部分国产品牌（MIUI、ColorOS、OriginOS 等）默认抑制通知横幅，或把通知排在其他通知下方。如果通知没有自动弹出，**下拉通知栏并下滑**找到 OpenCyvis 的通知即可。MIUI 用户可能还需要在系统设置中将通知样式切换为「安卓样式」。
</Callout>
...
<Callout type="warning">
MIUI 默认使用自定义通知样式，不支持内联回复。配对前请前往 **设置 → 应用 → OpenCyvis → 通知**，将通知样式切换为**安卓样式**——否则通知里的「点此输入配对码」输入框将不会显示。
</Callout>
</details>
